...
# -------------------------------------------------------------
    ...
    with col1:
        ...
        st.session_state.rainfall = st.slider("Rainfall (mm)", 20.0, 400.0, float(st.session_state.rainfall))
        ...
        st.session_state.sel_crop = st.selectbox("Cultivated Crop", crop_types, index=crop_types.index(st.session_state.sel_crop) if st.session_state.sel_crop in crop_types else 0)
...
# -------------------------------------------------------------
    ...
    c2.metric("Missing Values", "0 Nulls Found")
    c3.metric("Clustering Status", "Match Confirmed")
...
# -------------------------------------------------------------
    ...
    st.markdown("---")
    if st.button("⬅️ Back to Solution"):
        st.session_state.step = 6
        st.rerun()
